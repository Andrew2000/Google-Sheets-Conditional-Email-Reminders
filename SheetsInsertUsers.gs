...
function uploadData() {
  ...
  var userDataRange = ss.getDataRange();
  var userObjects = getRowsData(sheet, userDataRange,1);
  var runningLog = '<br>Uploaded following:<br><br>';
  for(var i = 1;i<userObjects.length;i++){
    var payload =  Utilities.jsonStringify(
      {"FirstName" : userObjects[i].firstname,
       "LastName" : userObjects[i].lastname,
       "Alias" : userObjects[i].alias,
       "Username" : userObjects[i].username,
       "UserRoleId" : userObjects[i].userroleid,
       "ProfileID" : userObjects[i].profileid,
       "LocaleSidKey" : userObjects[i].localesidkey,
       "LanguageLocaleKey" : userObjects[i].languagelocalekey,
       "EmailEncodingKey" : userObjects[i].emailencodingkey,
       "TimezoneSidKey" : userObjects[i].timezonesidkey,
       "CommunityNickname" : userObjects[i].communitynickname,
       "Email" : userObjects[i].email,
       "Team__c" : userObjects[i].team__c,
       "Start_Date__c" : userObjects[i].start_date__c,
       "Title" : userObjects[i].title,
       "Company" : userObjects[i].company,
       "Department" : userObjects[i].department,
       "Division" : userObjects[i].division,
       "Address" : userObjects[i].address
      }
    );
    Logger.log('trying ' + payload);
    var getDataURL = UserProperties.getProperty(baseURLPropertyName) + '/services/data/v26.0/sobjects/User/';
